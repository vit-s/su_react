...
let App = () => {
  return (
    <>
      ...
      {/*Product section two*/}
      <Section title={`Recommended`} classSection={`second_prod`} classSectionTwo={`second_prod_prod_block`}
               idSection={`recc_prod`}>
        <Product buyLink={`google.com`} buyLinkName={`Buy`} name={`Oranges`} price={100} qty={220}
